...
export class IniciarComponent {
  ...
  private buildForm() {
    this.myForm = this.fb.group({
      ...
      politic: ['', [Validators.required, Validators.requiredTrue]],
    });
  }

  registro() {
    this.conectarApiService
      .registroInteresado(this.myForm.value)
      .subscribe((data) => {
        if (data > 0) {
          alertify.alert(
            '¡Registro correcto!',
            'Enviamos los datos de acceso a tu cuenta de correo electrónico!',
            function () {
              alertify.success('Ok');
            }
          );
  ...
  account_validation_messages = {
    ...
    user_nombre: [
      ...
      {
        type: 'minlength',
        message: 'Su nombre debe tener al menos 6 caracteres',
      },
      {
        type: 'maxlength',
        message: 'Su nombre no puede tener más de 33 caracteres',
      },
